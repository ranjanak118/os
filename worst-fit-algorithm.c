...
int main()
{
	...
	int index=-1,delta=INT_MIN;
	for(i=0;i<num_process;i++)
	{
		index=-1,delta=INT_MIN;
		for(j=0;j<num_blocks;j++)
		{
			if((proc[i].size <= blk[j].fragment) && (blk[j].fragment - proc[i].size > delta))
			{
				delta = blk[j].fragment - proc[i].size;
				index = j;
			}
		}
		if(index!=-1)
		{
			proc[i].allocated = index;
			blk[index].fragment-=proc[i].size;
		}
	}
