...
int main()
{
	...
	if(choice==1)
	{
		...
		for(i=0;i<n+1;i++)
		{
			if(req[i].request_track_number > max_req)
			{
				max_req = req[i].request_track_number;
			}
		}
		seek_time+=(2*init_head);
		seek_time+=(max_req-init_head);
	...
	else if(choice==2)
	{
		...
		seek_time+=(2*(limit-init_head));
		seek_time+=(init_head - min_req);
